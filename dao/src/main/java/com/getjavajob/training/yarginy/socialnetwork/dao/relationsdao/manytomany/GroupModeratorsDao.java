...
public class GroupModeratorsDao implements ManyToManyDao<Account, Group> {
    ...
    public Collection<Account> selectBySecond(long groupId) {
        String query = "SELECT " + accountDao.getViewFields(ACC_ALIAS) + " FROM accounts acc JOIN groups_moderators " +
    ...
    public boolean relationExists(long accountId, long groupId) {
        String query = "SELECT 1 FROM groups_moderators gr_m WHERE gr_m.account_id = ? AND gr_m.group_id = ?";
        try {
    ...
    public boolean delete(long accountId, long groupId) {
        String query = "DELETE FROM groups_moderators gr_m WHERE gr_m.account_id = ? AND gr_m.group_id = ?";
