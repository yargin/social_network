...
public abstract class AbstractMessageDml extends AbstractDml<Message> {
    ...
    private final String selectAll = buildQuery().selectAll(table.table).build();
    ...
    private final String selectUpdateById = buildQuery().selectAll(table.table).where(table.id).build();
    private final String selectUpdateByAltKey = buildQuery().selectAll(table.table).where(table.author).and(
