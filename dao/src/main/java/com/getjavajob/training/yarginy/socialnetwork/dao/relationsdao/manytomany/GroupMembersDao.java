...
public class GroupMembersDao implements ManyToManyDao<Account, Group> {
    ...
    public Collection<Group> selectByFirst(long accountId) {
        String query = "SELECT " + groupDao.getViewFields(GROUP_ALIAS) + " FROM _groups gr JOIN groups_members " +
    ...
    public Collection<Account> selectBySecond(long groupId) {
        String query = "SELECT " + accountDao.getViewFields(ACC_ALIAS) + " FROM accounts acc JOIN groups_members " +
                "gr_m ON acc.id = gr_m.account_id WHERE gr_m.group_id = ?";
    ...
    public boolean relationExists(long accountId, long groupId) {
        String query = "SELECT 1 FROM groups_members gr_m WHERE gr_m.account_id = ? AND gr_m.group_id = ?";
        try {
    ...
    public boolean delete(long accountId, long groupId) {
        String query = "DELETE FROM groups_members gr_m WHERE gr_m.account_id = ? AND gr_m.group_id = ?";
